    ...
    public class ArithmeticController
    {
        ...
        public void HandleDragDrop(string sourceName, ImageDocument targetDoc)
        {
            ...
            using (Bitmap sourceBmp = _thumbController.FindThumbnailImageByName(sourceName))
            {
                ...
                {
                    case "Add":
                        resultBmp = _arithmeticService.AddImages(sourceBmp, targetDoc.CurrentBitmap);
                        opName = "Addition";
                        break;
                    case "Subtract":
                        resultBmp = _arithmeticService.SubtractImages(sourceBmp, targetDoc.CurrentBitmap);
                        opName = "Difference";
                        break;
                    case "Multiply":
                        resultBmp = _arithmeticService.MultiplyImages(sourceBmp, targetDoc.CurrentBitmap);
                        opName = "Multiplication";
                        break;
                    case "Divide":
                        resultBmp = _arithmeticService.DivideImages(sourceBmp, targetDoc.CurrentBitmap);
                        opName = "Division";
                        break;
                    case "AND":
                        resultBmp = _arithmeticService.AndImages(sourceBmp, targetDoc.CurrentBitmap);
                        opName = "AND";
                        break;
                    case "OR":
                        resultBmp = _arithmeticService.OrImages(sourceBmp, targetDoc.CurrentBitmap);
                        opName = "OR";
                        break;
                    case "XOR":
                        resultBmp = _arithmeticService.XorImages(sourceBmp, targetDoc.CurrentBitmap);
                        opName = "XOR";
                        break;
                }
