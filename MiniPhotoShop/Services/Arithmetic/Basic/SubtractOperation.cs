...
{
    public unsafe class SubtractOperation : TwoImageOperationBase
    {
        protected override void ProcessPixels(byte* pA, byte* pB, byte* pResult, int width, int height, int stride)
        {
            for (int y = 0; y < height; y++)
            {
                ...
                for (int x = 0; x < width; x++)
                {
                    ...
                    rowRes[i] = (byte)Math.Abs(rowB[i] - rowA[i]);
                    rowRes[i + 1] = (byte)Math.Abs(rowB[i + 1] - rowA[i + 1]);
                    rowRes[i + 2] = (byte)Math.Abs(rowB[i + 2] - rowA[i + 2]);
                    rowRes[i + 3] = 255;
                }
            }
        }
